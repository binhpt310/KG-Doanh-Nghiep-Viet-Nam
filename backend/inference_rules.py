...
Implements legal-threshold-based inference rules:
  ...
  R03 — Indirect Influence (upgraded with ownership thresholds)
  R04 — Shared major shareholder (cùng cổ đông lớn)
...
# ============================================================================
# SECTION 5 — Rule R03: Indirect Influence (Upgraded)
# ============================================================================
#
...
# Map influence level to relation label (Luật 3 / R03)
_R03_LABEL_MAP = {
...
def run_r03_indirect_influence(driver, batch_size: int = 500) -> int:
    """
    R03 — Indirect Influence (Upgraded with legal thresholds).
    ...
    while True:
        ...
        for rec in records:
            ...
            rel_label = _R03_LABEL_MAP[influence]
...
# ============================================================================
# SECTION 5b — Rule R04: Shared major shareholder (cùng cổ đông lớn)
# ============================================================================
#
...
def run_r04_shared_major_shareholder(driver, batch_size: int = 500) -> int:
...
def migrate_legacy_inferred_rule_ids(driver):
    """
    Normalize deprecated Neo4j inferred_from values (R07→R03, R12→R04).
    Idempotent; safe to run on every inference batch or at API startup.
    """
    counts = {}
    with driver.session() as session:
        for old, new in (("R07", "R03"), ("R12", "R04")):
            rec = session.run(
                """
                MATCH ()-[r]->()
                WHERE coalesce(r.inferred, false) = true AND r.inferred_from = $old
                SET r.inferred_from = $new
                RETURN count(r) AS c
                """,
                old=old,
                new=new,
            ...
            key = "%s_to_%s" % (old, new)
            counts[key] = int(rec["c"]) if rec and rec.get("c") is not None else 0
    return counts
...
def run_all_inference_rules(driver, batch_size: int = 500) -> dict:
    ...
    mig = migrate_legacy_inferred_rule_ids(driver)
    if sum(mig.values()):
        print(f"   ℹ️ Normalized legacy inferred_from: {mig}")
    ...
    # R03 — Indirect Influence (Upgraded)
    print("\n📌 R03: Indirect Influence (threshold-based)...")
    try:
        r03_count = run_r03_indirect_influence(driver, batch_size)
        results["R03_indirect_influence"] = r03_count
        print(f"   ✅ R03: Created {r03_count} indirect influence relations")
    except Exception as e:
        results["R03_indirect_influence"] = 0
        print(f"   ⚠️ R03 error: {e}")

    # R04 — Shared major shareholder (≥5% at two companies)
    print("\n📌 R04: Shared major shareholder...")
    try:
        r04_count = run_r04_shared_major_shareholder(driver, batch_size)
        results["R04_shared_major_shareholder"] = r04_count
        print(f"   ✅ R04: Created {r04_count} shared-major-shareholder links")
    except Exception as e:
        results["R04_shared_major_shareholder"] = 0
        print(f"   ⚠️ R04 error: {e}")
